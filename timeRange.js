(function () {
	...
	TimeSlider.prototype = {
		...
		createLayout : function (obj) {
			...
			var defaultOneTimeBlockTime = obj.defaultOneTimeBlockTime || 0; //手动点击创建时，每个时间段的默认时间 //mod by sw  2018年11月2日15:23:15
		...
		createTimeBlock : function (obj) {
			...
			//为时间段绑定点击事件
			//mod  by  sw  2018年10月23日14:31:37  delete 该事件
	...
	function creatEditDiv(context) {
		var editDivString = '\
            <div class="editWrap" id=<%="editDiv"+context.timeSliderNum%>>\
                <img src="../mod/images/add.png" title="新增" class="editImg"></img>\
                <img src="../mod/images/del.png" title="删除" class="delImg"></img>\
            </div>';
...
//		})
	...
	function removeAll(context) {
		var self = context;
		$($("#"+self.mountId).children()[0]).children().remove()
